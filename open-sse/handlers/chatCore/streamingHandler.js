 ...
 */
export function handleStreamingResponse({ providerResponse, provider, model, sourceFormat, targetFormat, userAgent, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, apiKeyId, clientRawRequest, onRequestSuccess, reqLogger, toolNameMap, streamController, onStreamComplete }) {
  ...
  saveRequestDetail(buildRequestDetail({
    provider, model, connectionId, apiKey, apiKeyId,
 ...
 */
export function buildOnStreamComplete({ provider, model, connectionId, apiKey, apiKeyId, requestStartTime, body, stream, finalBody, translatedBody, clientRawRequest }) {
  const streamDetailId = `${Date.now()}-${Math.random().toString(36).slice(2, 11)}`;
  ...
  const onStreamComplete = (contentObj, usage, ttftAt) => {
    ...
    saveUsageStats({ provider, model, tokens: usage, connectionId, apiKey, apiKeyId, endpoint: clientRawRequest?.endpoint, label: "STREAM USAGE" });
  };
